...
def generate_sample_documents(count: int, with_vectors: bool = False) -> list[tuple]:
    ...
    for i in range(count):
        ...
        metadata = json.dumps({"type": random.choice(categories), "priority": random.randint(1, 5)})
...
def bulk_insert_execute_values(
    conn, data: list[tuple], with_vectors: bool = False, batch_size: int = 1000
) -> float:
    """
...
def bulk_insert_copy(conn, data: list[tuple], with_vectors: bool = False) -> float:
    """
...
def bulk_insert_batch(
    conn, data: list[tuple], with_vectors: bool = False, batch_size: int = 100
) -> float:
    """
...
def main():
    parser = argparse.ArgumentParser(description="Demonstrate bulk insert patterns for PostgreSQL")
