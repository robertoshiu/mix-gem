...
def similarity_search(
    conn, embedding: list[float], limit: int = 10, distance_type: str = "cosine"
) -> list[dict]:
    """
...
def hybrid_search(
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            LIMIT %s
        """,
            (query_text, query_embedding, query_embedding, text_weight, vector_weight, limit),
        )
        return cur.fetchall()
...
def check_index_stats(conn, table_name: str = "documents") -> dict:
    ...
    with conn.cursor() as cur:
        ...
        return {"indexes": indexes, "vector_count": vector_count}
...
def tune_search_params(conn, target_recall: float = 0.95):
    ...
    if target_recall > 0.95:
        if ef_search < 100:
            suggestions.append(
                f"Increase hnsw.ef_search from {ef_search} to 100-200 for better recall"
            )
        if probes < 10:
            suggestions.append(f"Increase ivfflat.probes from {probes} to 10-20 for better recall")

    return {"current_ef_search": ef_search, "current_probes": probes, "suggestions": suggestions}
...
def main():
    parser = argparse.ArgumentParser(description="pgvector search helpers and examples")
