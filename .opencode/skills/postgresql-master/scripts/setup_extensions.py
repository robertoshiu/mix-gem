...
def install_extension(cur, name: str) -> bool:
    ...
    try:
        cur.execute(sql.SQL("CREATE EXTENSION IF NOT EXISTS {}").format(sql.Identifier(name)))
        return True
...
def main():
    ...
    parser.add_argument(
        "--include-optional", action="store_true", help="Try to install optional extensions"
    )
    ...
    for ext_name, description in extensions_to_install:
        ...
        if ext_name in installed:
            ...
            print("  Not available on this PostgreSQL installation")
    ...
    print(
        f"Failed/Unavailable: {len(results['failed'])} ({', '.join(results['failed']) or 'none'})"
    )
