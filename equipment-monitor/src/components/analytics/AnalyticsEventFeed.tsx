...
export function AnalyticsEventFeed({ events, currentTick }: AnalyticsEventFeedProps) {
  ...
  // Track the newest event id rather than length: once the ring buffer
  // saturates at ANALYTICS_EVENT_CAPACITY, events.length stays pinned and a
  // length-based guard stops firing even though a new event arrives each tick.
  ...
  // Auto-scroll to top when a new event arrives (unless user scrolled).
  // Events flow oldest-first into this component, so the newest id sits at
  // the tail. The store re-tags every tick with `-e<elapsed>` so the id
  // changes on every push, including post-saturation wraps.
  const newestId = events.length > 0 ? events[events.length - 1].id : null;
  useEffect(() => {
    if (newestId && newestId !== prevNewestIdRef.current && !userScrolledRef.current) {
      const el = listRef.current;
