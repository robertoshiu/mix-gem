...
function createProcessTools(scene: BABYLON.Scene, mode: FabTwinMode, faultId: FabTwinFaultId) {
  FAB_TWIN_TOOLS.forEach((tool) => {
    ...
    attachMetadata(port, `${tool.tool_id}-load-port`, `${tool.path}/INTERACTIVE/LOAD_PORT`, 'interactive-load-port', ['process'], { state: mode === 'lot-transfer' ? 'transferring' : 'ready' });
    ...
    panel.rotation.y = mode === 'maintenance' ? -0.82 : 0;
    panel.material = createPbr(scene, `${tool.tool_id}-service-mat`, tool.color, 0.34, 0.3);
    attachMetadata(panel, `${tool.tool_id}-door-panel`, `${tool.path}/INTERACTIVE/SERVICE_DOOR`, 'door-open-panel', ['process'], { open: mode === 'maintenance', serviceSide: tool.serviceSide });
    ...
    if ((mode === 'alarm' || faultId === 'single-tool-down') && tool.tool_id === 'ETCH-ICP-02') {
      const alarm = BABYLON.MeshBuilder.CreateSphere(`${tool.tool_id}-alarm-flash`, { diameter: 0.58, segments: 18 }, scene);
      alarm.parent = group;
      alarm.position = new BABYLON.Vector3(0, tool.sizeM[1] + 0.42, 0);
      const alarmMat = createPbr(scene, `${tool.tool_id}-alarm-mat`, '#ef4444', 0.22, 0.1);
      alarmMat.emissiveColor = BABYLON.Color3.FromHexString('#ef4444');
      alarm.material = alarmMat;
      attachMetadata(alarm, `${tool.tool_id}-alarm-flash`, `${tool.path}/ALARM`, 'alarm-beacon', ['process'], { faultId });
    }
  });
}
...
function createTransport(scene: BABYLON.Scene, mode: FabTwinMode, faultId: FabTwinFaultId) {
  const points = [
  ...
  attachMetadata(carrier, 'FOUP-CARRIER-A17', '/FAB1/L1/AMHS/CARRIERS/FOUP-CARRIER-A17', 'lot-carrier', ['transport'], { lot: 'LOT-2026-0412', wafers: 25, state: mode === 'lot-transfer' ? 'transferring' : 'waiting' });
...
function createScene(canvas: HTMLCanvasElement, props: WarRoomBabylonSceneProps) {
  ...
  createProcessTools(scene, props.mode, props.faultId);
  createSubsystemEquipment(scene, props.faultId);
  createSensors(scene, props.faultId);
  createTransport(scene, props.mode, props.faultId);
