---
name: "source-command-chat-history-cloud-status"
description: "Check cloud sync status and connection info"
---

# source-command-chat-history-cloud-status

Use this skill when the user asks to run the migrated source command `chat-history-cloud-status`.
...
1. Check if `.Codex/chat-history-enabled` exists:
   - If yes: "Local logging: ON"
   - If no: "Local logging: OFF"

2. Check if `.Codex/chat-history-cloud-enabled` exists:
   - If yes: "Cloud sync: ON"
   - If no: "Cloud sync: OFF"

3. If cloud sync is ON, read `.Codex/chat-history-cloud-config.json` and display:
