...
export function ViolationCard({ violation, onAcknowledge }: ViolationCardProps) {
  ...
  return (
    <div
      className="bg-red-950/30 border border-[var(--smartfactory-status-red)] rounded p-3 flex flex-col gap-2"
      style={acknowledged ? {} : { boxShadow: '0 0 12px rgba(239,68,68,0.3)' }}
    >
      <div className="flex items-start gap-2">
        <AlertTriangle className="w-4 h-4 text-[var(--smartfactory-status-red)] mt-0.5 shrink-0" />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-[var(--smartfactory-status-red)]">{RULE_LABELS[rule]}</p>
      ...
      {acknowledged ? (
        ...
        <button
          ...
          className="min-h-[44px] px-3 py-2 rounded text-sm font-semibold bg-[var(--smartfactory-surface-elevated)] hover:bg-[var(--smartfactory-border-default)] border border-[var(--smartfactory-status-red)] text-[var(--smartfactory-text-primary)] cursor-pointer transition-colors"
